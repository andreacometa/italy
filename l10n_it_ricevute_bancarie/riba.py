...
class riba_list(orm.Model):
    ...
    _columns = {
        ...
        'config': fields.many2one(
            'riba.configuration', 'Configuration', select=True, required=True,
        ...
        'type': fields.related('config', 'tipo', type='char', size=32,
    ...
    _defaults = {
        ...
        'name': (
            lambda self, cr, uid, context: self.pool[
                'ir.sequence'].get(cr, uid, 'riba.list')),
        'company_id': (
            lambda self, cr, uid, c: self.pool[
                'res.company']._company_default_get(cr, uid, 'riba.list',
                                                    context=c)),
...
class riba_list_line(orm.Model):
    ...
    def move_line_id_payment_gets(self, cr, uid, ids, *args):
        ...
        cr.execute("""
            ...
            FROM account_move_line l LEFT JOIN riba_list_line list_line ON
    ...
    _columns = {
        ...
        'move_line_ids': fields.one2many(
            'riba.list.move.line', 'riba_line_id', 'Credit move lines'),
        ...
        'bank_id': fields.many2one(
            'res.partner.bank', 'Debitor Bank'),
        'iban': fields.related(
            'bank_id', 'iban', type='char', string='IBAN', store=False,
            readonly=True),
        'list_id': fields.many2one(
            'riba.list', 'list', required=True, ondelete='cascade'),
...
payment."),
        'payment_ids': fields.function(
            ...
            string='Payments'),
        'type': fields.related(
            'list_id', 'type', type='char', size=32, string='Type',
            readonly=True),
    }
...
class riba_list_move_line(orm.Model):
    ...
    _columns = {
        ...
        'riba_line_id': fields.many2one(
            'riba.list.line', 'list line', ondelete='cascade'),
    }
